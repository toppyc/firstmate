...
A run found on the crew's own branch that no reported head binds is refused but reported, with `run-unbound` replacing a verdict that would otherwise have named no source at all, so a supervisor can tell a vanished worker from a refused match without reading `axi status` by hand.
