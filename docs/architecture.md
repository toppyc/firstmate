...
That recording exists only so teardown can verify landed work, so a PR firstmate opened itself for a backlog item that never had a worker has no consumer for it and merges through the same helper's verified opt-in instead of a second merge path.
