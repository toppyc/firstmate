...
A crew that declares `paused:` for a known external wait is separately absorbed while that declaration stays current and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, even when a running pipeline makes its authoritative current state read working.
The declaration goes stale once it has gone `FM_PAUSE_RESURFACE_SECS` without being re-declared, and a working crew then returns to ordinary wedge escalation, so one forgotten `paused:` line cannot mute a wedge indefinitely.
...
`bin/fm-nm-run-lib.sh` owns which of a run's reported heads may bind it to a worktree and the exact head ancestry rules; the `bin/fm-crew-state.sh` header owns the rest of the mapping.
A run found on the crew's own branch that no reported head binds is refused, and any verdict that still reports `unknown` says why - with `run-unbound` replacing a source that would otherwise have been none - so a supervisor can tell a vanished worker from a refused match without reading `axi status` by hand; a verdict that names a real state is already an answer and stays unannotated.
...
Both readers of that recording, teardown's landed-work verification and `bin/fm-review-diff.sh`'s base resolution, reach it only through the task's metadata, so a PR firstmate opened itself for a backlog item that never had a worker has no consumer for it and merges through the same helper's verified opt-in instead of a second merge path.
Absent metadata never relaxes the guard on its own: the opt-in refuses whenever any per-task worker record is present, including the one artifact teardown does not remove, which is what separates a task that never had a worker from a crew-shipped task whose metadata has gone missing.
