...
Absent metadata never relaxes the guard on its own: the opt-in refuses whenever any durable per-task worker record survives, which is what separates a task that never had a worker from a crew-shipped task whose metadata has gone missing.
