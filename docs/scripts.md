...
| `fm-pr-merge.sh`         | Record PR metadata (except the verified no-worker path), then merge a task's canonical full GitHub URL |
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode |
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, require completed scout deliverables, retire secondmate homes |
| `fm-resource.sh`         | Worker-facing `record`/`forget`/`list` of the external resources a task created, so teardown releases exactly those |
| `fm-resource-lib.sh`     | Owner of the per-task resource record format, accepted kinds and names, and the release contract |
