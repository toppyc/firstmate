...
FM_RESOURCE_DOCKER_TIMEOUT=20   # seconds allowed per docker call when fm-teardown.sh releases a task's recorded containers; a blank, non-numeric, or zero value asks docker nothing and reports the call as timed out
FM_RESOURCE_LOCK_WAIT_SECS=10   # seconds bin/fm-resource.sh waits for another writer of the same task's resource record before reporting an error rather than blocking the worker further
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared external wait re-surfaces for a recheck in the watcher or away-mode daemon, and equally how long a current declared pause overrides an authoritative working verdict, so raising it also extends how long wedge escalation is held off for a crew whose pipeline is running
