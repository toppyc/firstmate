...
state/               runtime records and signals; gitignored
  ...
  <id>.resources   external resources the task's own worker created (a service container it started), recorded by bin/fm-resource.sh so teardown can stop exactly those and never guess a name; retained when one could not be released
...
Use `bin/fm-pr-merge.sh` for every task PR merge so merge metadata is recorded, except on its verified no-worker path where a firstmate-opened PR has no task record to record against, and use `bin/fm-merge-local.sh` for approved local-only landing; never call a lower-level merge command around their guards.
After an autonomous merge, give the captain a one-line full-URL or local-main outcome.

### Validate

For a no-mistakes ship, the generated brief has the implementation worker start validation itself after its own implementation commit, so no firstmate relay is expected and its only ready signal is the terminal one below.
Send that invocation yourself, using the harness invocation owned by `harness-adapters`, only when a worker has stopped without starting it.
