...
#
# Every fixture in this suite comes from here, so teardown's sourcing contract is
# enumerated ONCE. A second hand-maintained copy of this list drifts out of date
# the first time teardown gains a sibling and then fails on the `.` instead of on
# what its test is about. An empty <tasktmp> writes a meta with no tasktmp= line,
# which is the pre-fix shape.
make_fake_root() {
  ...
  # fm-resource-lib.sh: teardown sources it to release the external resources a
  # task recorded for itself, and fm-timeout-lib.sh bounds the docker calls that
  # release makes. Nothing is recorded in this fixture, but both siblings are
  # required.
  ln -s "$ROOT/bin/fm-resource-lib.sh" "$fake/bin/fm-resource-lib.sh"
